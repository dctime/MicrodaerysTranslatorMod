...
public class ScreenShotSelectAreaScreen extends Screen {
    ...
    public ScreenShotSelectAreaScreen(NativeImage image, Screen oldScreen) {
        super(new StringTextComponent("screenshotselectarea"));
...
//        RenderSystem.setShader(GameRenderer::getPositionTexShader);
        Minecraft.getInstance().getTextureManager().bind(tempImageResourceLocation);

        blit(poseStack, 0, 0, 0, 0, (int)(tempImage.getWidth()/scaledToWindowWidthRatio), (int)(tempImage.getHeight()/scaledToWindowHeightRatio), (int)(tempImage.getWidth()/scaledToWindowWidthRatio), (int)(tempImage.getHeight()/scaledToWindowHeightRatio));
        if (mouseDragged)
            fill(poseStack, (int)(mouseX1), (int)(mouseY1), mouseX, mouseY, 0x80FFFFFF);
    }
...
//                toImage.setPixelRGBA(pXFrom + pXToDelta + k, pYFrom + pYToDelta + l, i1);
    ...
    private void processImageAndRequest() {
        ...
        NativeImage cropTempImage = new NativeImage(NativeImage.PixelFormat.RGBA, endX-startX, endY-startY, false);
        ...
        try {
            ...
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.displayClientMessage(new StringTextComponent("螢幕截圖檔案無法翻成Base64" + e.getMessage()).withStyle(TextFormatting.RED), false);
            }
...
//            tempBase64Image = null;
        ...
        if (Translator.translating) {
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.displayClientMessage(new StringTextComponent("翻譯器正在忙碌中，請稍後再截圖。").withStyle(TextFormatting.YELLOW), false);
        ...
        try {
            ...
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.displayClientMessage(new StringTextComponent("螢幕截圖翻譯中...").withStyle(TextFormatting.GREEN), false);
                ScreenEventRender.setRenderText("螢幕截圖翻譯中...");
            }
        } catch (IOException | InterruptedException e) {
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.displayClientMessage(new StringTextComponent("螢幕翻譯錯誤: " + e.getMessage()).withStyle(TextFormatting.RED), false);
            }
        }
    }
}
